...
class _PagedDataTableFilterTab<TKey extends Object, TResult extends Object> extends StatelessWidget {
  ...
  const _PagedDataTableFilterTab(this.menu, this.header, this.filterDialogWidth,
      {this.customFilterPageOnPressed, this.hideDefaultFilterIcon = false,});
  ...
  Widget build(BuildContext context) {
    ...
    Widget child = SizedBox(
      ...
      child: Consumer<_PagedDataTableState<TKey, TResult>>(builder: (context, state, child) {
        return Row(
          ...
          children: [
            Expanded(
              child: Row(
                children: [
                  /* FILTER BUTTON */
                  if (state.filters.isNotEmpty && !hideDefaultFilterIcon)
                    Ink(
                      child: InkWell(
                        radius: 20,
                        child: Tooltip(
                          ...
                          child: MouseRegion(
                            ...
                            child: GestureDetector(
                              ...
                              child: Padding(
                                ...
                                child: Icon(Icons.filter_list_rounded, color: theme.buttonsColor),
                              ),
                            ),
                          ),
                        ),
                  ...
                  if (customFilterPageOnPressed != null)
                    Ink(
                      child: InkWell(
                        radius: 20,
                        child: Tooltip(
                          ...
                          child: MouseRegion(
                            ...
                            child: GestureDetector(
                              onTapDown: state.tableState == _TableState.loading
                                  ? null
                                  : (details) {
                                customFilterPageOnPressed?.call();
                              },
                              child: Padding(
                                padding: const EdgeInsets.symmetric(horizontal: 16.0),
                                child: Icon(Icons.filter_list_rounded, color: theme.buttonsColor),
            ...
            Row(
              children: [
                if (header != null) ...[const Spacer(), Flexible(child: header!)] else
  ...
  Future<void> _showFilterOverlay(TapDownDetails details, BuildContext context,
      _PagedDataTableState<TKey, TResult> state, PagedDataTableLocalization localizations) async {
    ...
    await showDialog(
        ...
        builder: (context) =>
            _FiltersDialog<TKey, TResult>(
              rect: rect,
              state: state,
              localizations: localizations,
              dialogWidth: filterDialogWidth,
            ));
  }
}
...
class _FiltersDialog<TKey extends Object, TResult extends Object> extends StatelessWidget {
  ...
  const _FiltersDialog({required this.rect, required this.state, required this.localizations, this.dialogWidth});
